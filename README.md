...
`aicli` 是一个用 Rust 写的命令行助手：你用自然语言描述想做的事，它调用模型生成对应 shell 命令或短脚本，先展示给你确认。你可以直接回车执行，也可以改完再执行。
...
不加 `-y` 时，`aicli` 会进入可编辑确认流程；加 `-y` 时会直接执行生成的命令或脚本。

如果模型生成的是多行脚本，输出会显示为 `Script` 块：
...
- 直接回车：执行脚本
- 输入 `e`：用 `$VISUAL` / `$EDITOR` 打开临时文件编辑，保存退出后执行
- 输入 `q`：取消
...
- 模型也可以生成短 shell 脚本；脚本会整体交给当前 shell 执行。
